...
const Home = () => {
  ...
  const navigationCards = [
    ...
    },
    {
      ...
      description: 'Streamline your appointment scheduling and management process',
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const performIntelligentSearch = (query) => {
    ...
    callCenterRebuttals.forEach(rebuttal => {
  ...
  const handleDispositionClick = (dispositionName) => {
    ...
    const relevantRebuttals = callCenterRebuttals.filter(rebuttal => {
  ...
  const handleMoreOptions = (rebuttal) => {
    ...
    const allCategoryRebuttals = callCenterRebuttals.filter(r => 
      r.category === rebuttal.category && r.id !== rebuttal.id
    );
    
    // Find related rebuttals based on tags
    const relatedByTags = callCenterRebuttals.filter(r => 
  ...
  const handleSimpleModeItemClick = (item) => {
    // Find matching rebuttal in callCenterRebuttals
    const matchingRebuttal = callCenterRebuttals.find(rebuttal => 
                ...
                {callCenterRebuttals
                  ...
                  .map((rebuttal, index) => (
                    <div key={index} className="objection-item">
                      ...
                      <p>{typeof rebuttal.content === 'string' ? rebuttal.content : rebuttal.content.pt1}</p>
                    </div>
                  ))}
              </div>
            </div>
                      ...
                      <>
                        <div className="rebuttal-part">
                          ...
                          <p>{typeof selectedSimpleModeItem.content === 'string' 
                            ? selectedSimpleModeItem.content 
                            : selectedSimpleModeItem.content.pt1}</p>
                        </div>
                        <div className="rebuttal-part">
                          <h4>Part 2: Follow-up</h4>
                          <p>{typeof selectedSimpleModeItem.content === 'string'
                            ? selectedSimpleModeItem.content
                            : selectedSimpleModeItem.content.pt2}</p>
                        </div>
                      </>
                    )}
  ...
  const findBestMatch = (query) => {
    // Search in rebuttals
    const rebuttalMatch = callCenterRebuttals.find(rebuttal => 
  ...
  // Update the simple mode categories to use callCenterRebuttals data
  const simpleModeCategories = [
    {
      id: 'spouse-related',
      title: 'Spouse Related',
      icon: '💑',
      color: '#4f46e5',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'spouse-consultation' || 
        rebuttal.category === 'spouse'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle spouse consultation needs'
      }))
    },
    {
      id: 'decision-maker',
      ...
      color: '#0ea5e9',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'one-legger'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle single decision maker situations'
      }))
    },
    {
      id: 'time-busy',
      ...
      color: '#22c55e',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'time-concern'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle time and busy schedule concerns'
      }))
    },
    {
      id: 'callback',
      ...
      items: [
        {
          id: 'callback-1',
          title: 'Standard Callback Request',
          category: 'callback',
          description: 'Handle basic callback requests professionally',
          content: {
            pt1: "I understand you'd like a callback. I'd be happy to call you back at a more convenient time. When would be the best time to reach you?",
            pt2: "Great, I've noted that time. I'll call you back at [time] on [date]. Is there anything specific you'd like me to prepare for our conversation?"
          },
          steps: [
            'Confirm the best time for callback',
            'Get alternative contact methods',
        ...
        {
          id: 'callback-2',
          title: 'Urgent Callback Needed',
          category: 'callback',
          description: 'Handle urgent callback requests with priority',
          content: {
            pt1: "I understand this is urgent. I can call you back within the next [time frame]. Would that work for you?",
            pt2: "I'll make sure to prioritize your callback. In the meantime, is there any specific information you'd like me to gather before calling you back?"
          },
          steps: [
            'Acknowledge urgency',
            'Offer immediate callback window',
            'Gather necessary information',
            'Set priority callback time'
          ],
          tips: [
            'Respond quickly to urgent requests',
            'Keep the customer informed of status',
            'Follow up immediately if missed'
          ]
        },
        {
          id: 'callback-3',
          title: 'Callback with Specific Time',
          category: 'callback',
          description: 'Handle callback requests with specific time preferences',
          content: {
            pt1: "I see you'd prefer a callback at [specific time]. I'll make sure to call you then. Is there anything else I should know before our callback?",
            pt2: "Perfect, I've scheduled your callback for [specific time]. I'll be prepared with all the information you need."
          },
          steps: [
            'Confirm specific time preference',
            'Note any special requirements',
            'Schedule the callback',
            'Prepare for the callback'
          ],
          tips: [
            'Respect their time preference',
        ...
        {
          id: 'callback-4',
          title: 'Callback with Alternative Contact',
          category: 'callback',
          description: 'Handle callback requests with alternative contact methods',
          content: {
            pt1: "I understand you'd prefer to be contacted through [alternative method]. I can definitely accommodate that. What's the best way to reach you?",
            pt2: "I'll make sure to contact you through [alternative method] at [time]. Is there anything specific you'd like me to prepare for our conversation?"
          },
          steps: [
            'Confirm alternative contact method',
            'Verify contact details',
            'Set callback time',
            'Document preferred method'
          ],
          tips: [
            'Respect their contact preference',
    ...
    },
    {
      ...
      color: '#8b5cf6',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'reset-appt'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle rescheduling requests'
      }))
    },
    {
      id: 'price-phone',
      ...
      items: callCenterRebuttals.filter(rebuttal => 
        ...
        ...rebuttal,
        description: rebuttal.description || 'Handle price inquiries over phone'
      }))
    },
    {
      id: 'budget',
      ...
      color: '#ec4899',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'cant-afford'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle budget and affordability concerns'
      }))
    },
    {
      id: 'not-interested',
      ...
      color: '#dc2626',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'not-interested'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle general disinterest'
      }))
    },
    {
      id: 'curious',
      ...
      color: '#14b8a6',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'curious'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle information gathering inquiries'
      }))
    },
    {
      id: 'no-request',
      ...
      color: '#6b7280',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'no-request'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle no request situations'
      }))
    },
    {
      id: 'bad-reviews',
      ...
      color: '#f97316',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'bad-reviews'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle negative review concerns'
      }))
    },
    {
      id: 'government',
      ...
      color: '#3b82f6',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'government-grants'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle government grant inquiries'
      }))
    },
    {
      id: 'repair',
      ...
      color: '#10b981',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'repair'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle repair-only requests'
      }))
    },
    {
      id: 'not-ready',
      ...
      color: '#6366f1',
      items: callCenterRebuttals.filter(rebuttal => 
        rebuttal.category === 'not-ready'
      ).map(rebuttal => ({
        ...rebuttal,
        description: rebuttal.description || 'Handle not ready situations'
      }))
    }
  ];
        ...
        {isSimpleMode ? (
          <div className="simple-mode-container">
                ...
                {simpleModeCategories.map(category => (
                  <button
                    ...
                    style={{ 
                      ...
                      '--category-gradient': `linear-gradient(135deg, ${category.color}, ${adjustColor(category.color, -20)})`
                    }}
                  >
                    <div className="quick-nav-card-content">
                      ...
                      <div className="quick-nav-info">
                        ...
                        <p className="quick-nav-description">
                          {category.id === 'spouse-related' ? 'Joint Decision Making' :
                           category.id === 'decision-maker' ? 'Individual Decisions' :
                           category.id === 'time-busy' ? 'Time Management' :
                           category.id === 'callback' ? 'Follow-up Process' :
                           category.id === 'reschedule' ? 'Rescheduling' :
                           category.id === 'price-phone' ? 'Price Discussion' :
                           category.id === 'budget' ? 'Budget Planning' :
                           category.id === 'not-interested' ? 'Interest Building' :
                           category.id === 'curious' ? 'Information Gathering' :
                           category.id === 'no-request' ? 'Request Handling' :
                           category.id === 'bad-reviews' ? 'Review Management' :
                           category.id === 'government' ? 'Government Programs' :
                           category.id === 'repair' ? 'Repair Services' :
                           'Not Ready'}
                        </p>
                      </div>
                    </div>
            ...
            <div className="simple-mode-grid">
              {simpleModeCategories.map(category => (
                <div key={category.id} className="simple-mode-category" data-category={category.id}>
